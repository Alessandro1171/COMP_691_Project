...
class StatusBrainV1(sb.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        features, len = batch.sig

        features = self.modules.compute_features(features)

        norm_features = self.modules.mean_var_norm(features, lengths=len)

        embeddings = self.modules.embedding_model(norm_features,len)
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage == sb.Stage.TRAIN and hasattr(self.modules, "env_corrupt"):
            ...
            status, lens= torch.cat([status, status], dim=0),torch.cat([lens, lens])
...
def dataio_prep(hparams):
    ...
    def label_pipeline(status):
        """Defines the pipeline to process the status labels.
...
if __name__ == "__main__":
    ...
    status_brainV1.fit(
        epoch_counter=status_brainV1.hparams.epoch_counter,
    ...
    test_stats = status_brainV1.evaluate(
